...
const aphroditeNavItems = [
  ...
  { id: "currency", href: "/dashboard/networks/aphrodite/currency", label: "Валюты", icon: Banknote },
  { id: "crypto", href: "/dashboard/networks/aphrodite/crypto", label: "Крипта", icon: Bitcoin },
  { id: "metals", href: "/dashboard/networks/aphrodite/metals", label: "Металлы", icon: Gem },
  { id: "studio", href: "/dashboard/networks/aphrodite/studio", label: "Студия", icon: Clapperboard },
  { id: "legacy", href: "/dashboard/networks/aphrodite/legacy", label: "Старая сеть", icon: RadioTower },
];

const zodiacNavItems = [
  { id: "overview", href: "/dashboard/networks/zodiac", label: "Каналы Зодиака", icon: RadioTower },
...
const zodiacAdvancedNavItems = [
  ...
  { id: "channels", href: "/dashboard/networks/zodiac/channels", label: "Реестр 13 каналов", icon: RadioTower },
...
export function Sidebar() {
  const pathname = usePathname();
  const advancedZodiacOpen = zodiacAdvancedNavItems.some((item) => pathname === item.href);
  ...
  return (
    <aside className="border-b border-line bg-[#08101f]/95 backdrop-blur lg:fixed lg:inset-y-0 lg:left-0 lg:z-40 lg:w-72 lg:overflow-y-auto lg:border-b-0 lg:border-r">
      <div className="flex min-h-full flex-col px-4 py-4 lg:px-5 lg:py-6">
        ...
        {/* Aphrodite Operator Platform (top level) */}
        <Link
          href="/dashboard/networks/aphrodite"
          className="mt-2 mb-6 flex items-center gap-3"
          data-aphrodite-operator-home
        >
          ...
          <div>
            ...
            <p className="text-[11px] uppercase leading-tight tracking-wider text-slate-500">Operator Platform</p>
          </div>
        </Link>

        <nav
          className="mb-8 grid grid-cols-2 gap-1 lg:grid-cols-1"
          aria-label="Афродита Operator Platform"
          data-aphrodite-operator-nav
        >
        ...
        {/* Zodiac is a module inside Aphrodite, not the top-level platform. */}
        <div className="border-t border-slate-800/60 pt-6" data-zodiac-operator-module>
          <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-cyan-200">МОДУЛЬ ZODIAC</p>

          <nav className="grid grid-cols-2 gap-1 lg:grid-cols-1" aria-label="Модуль Zodiac">
            {zodiacNavItems.map((item) => {
              const Icon = item.icon;
              const active =
                pathname === item.href ||
                (item.href !== "/dashboard/networks/zodiac" && pathname.startsWith(item.href));
          ...
          <div className="mt-3">
            <NavGroup
              ...
              items={zodiacAdvancedNavItems}
              defaultOpen={advancedZodiacOpen}
              pathPrefix="/dashboard/networks/zodiac/__advanced"
