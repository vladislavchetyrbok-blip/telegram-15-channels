...
export function ZodiacDateInput({
  ...
  const inputClass = publicMode
    ? `aphrodite-touch-target w-full rounded-lg border bg-white/8 px-3 pr-12 text-[16px] text-white placeholder-slate-400 outline-none transition focus:border-violet-300 focus:bg-white/10 ${hasError ? "border-rose-300" : "border-white/15"}`
    : `aphrodite-touch-target w-full rounded-lg border bg-white px-3 pr-12 text-[16px] text-slate-900 placeholder-slate-400 outline-none transition focus:border-violet-500 ${hasError ? "border-rose-300" : "border-slate-200"}`;
  ...
  return (
    <div className="min-w-0 max-w-full">
      ...
      <p className={publicMode ? "aphrodite-wrap-anywhere mt-1 text-xs leading-5 text-slate-400" : "aphrodite-wrap-anywhere mt-1 text-xs leading-5 text-slate-500"}>{resolvedHint}</p>
    </div>
  );
}
