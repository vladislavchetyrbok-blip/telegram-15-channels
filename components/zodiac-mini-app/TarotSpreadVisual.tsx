  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-amber-100">Tarot spread</p>
      ...
      <div className={`mt-4 grid gap-3 ${spread.cards.length === 1 ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-3"}`}>
        {spread.cards.map((item, index) => (
          <article
            key={`${item.key}-${item.position}`}
            className="relative min-h-[210px] overflow-hidden rounded-xl border border-white/12 bg-white/[0.06] p-3"
          ...
          >
            ...
            <div className="relative mx-auto flex h-32 w-24 flex-col items-center justify-between rounded-[18px] border border-amber-100/35 bg-gradient-to-b from-amber-100/20 via-white/8 to-indigo-900/30 p-3 text-center shadow-inner">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-100">{String(index + 1).padStart(2, "0")}</span>
              <span className="text-4xl text-amber-50">{arcanaMarks[index % arcanaMarks.length]}</span>
              <span className="text-[10px] font-semibold uppercase tracking-wide text-indigo-100">arcana</span>
            </div>
            <div className="relative mt-3 text-center">
              ...
              <p className="mt-2 text-xs leading-5 text-slate-300">{item.card.mainMeaning}</p>
            </div>
          </article>
        ))}
      </div>
