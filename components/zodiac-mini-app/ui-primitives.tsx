...
export function FeatureCard({ publicMode, title, subtitle, children }: { publicMode: boolean; title: string; subtitle: string; children: ReactNode }) {
  return (
    <div className={publicMode ? "min-w-0 max-w-full overflow-hidden rounded-lg border border-white/12 bg-white/8 p-3 text-slate-100 min-[390px]:p-4" : "min-w-0 max-w-full overflow-hidden rounded-lg border border-slate-200 bg-white p-3 text-slate-700 min-[390px]:p-4"}>
      <p className={publicMode ? "aphrodite-wrap-anywhere text-base font-semibold text-white" : "aphrodite-wrap-anywhere text-base font-semibold text-slate-950"}>{title}</p>
...
export function panelClass(publicMode: boolean) {
  return publicMode
    ? "min-w-0 max-w-full overflow-hidden rounded-lg border border-white/12 bg-white/10 p-3 shadow-[0_18px_60px_rgba(8,13,30,0.38)] backdrop-blur transition-all duration-300 min-[390px]:p-4"
