import { reviewVisualRegenerationDraft } from "./lib/visual-regeneration-drafts.mjs";

const args = parseArgs(process.argv.slice(2));
const action = getAction(args);

if (!action) {
  console.error("Choose exactly one action: --approve, --reject, or --needs-changes. No visual drafts were changed.");
  process.exit(1);
}

if (!args.draftId) {
  console.error("Missing required --draft-id=visual_draft_... value. No visual drafts were changed.");
  process.exit(1);
}

const result = await reviewVisualRegenerationDraft({
