...
import { buildProviderCandidateFields, enrichCandidateProviderFields } from "./visual-provider-system.mjs";
...
export async function getVisualRegenerationImageCandidateStatus() {
  ...
  const candidatesByProvider = candidates.reduce((acc, candidate) => {
  ...
  return {
    ...
    summary: {
      ...
      notPremium: candidates.filter((candidate) => candidate.visualQualityStatus === "notPremium").length,
    },
...
function buildCandidateMetadata({ draft, candidatePath, generationStatus, generatorUsed }) {
  const providerFields = buildProviderCandidateFields({
    ...
    generatorUsed,
    visualQualityEstimate: draft.scores?.expectedAfter ?? {},
    sourceNote: "Generated by the local placeholder/demo PNG generator.",
  });
  return {
    draftId: draft.id,
    postId: draft.postId,
    ...
    visualQualityEstimate: {
      ...
      qualityStatus: providerFields.visualQualityStatus,
      issues: providerFields.placeholderProviderUsed ? ["placeholderProviderUsed"] : [],
    },
...
function withFileState(candidate) {
  ...
  return {
    ...enrichCandidateProviderFields(candidate),
