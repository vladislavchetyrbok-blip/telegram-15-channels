...
export function runAphroditeOwnerManualFinalGatePackageQa(config) {
  passed = 0;
  failed = 0;
  ...
  check("current main head recorded", config.model.currentMainHead === "b40080ce9c956cde622f96a185bc8cde5c604201");
  ...
  check("safety boundaries documented", Array.isArray(config.model.safetyBoundaries) && config.model.safetyBoundaries.length >= 5);
  check("next package documented", config.model.nextPackageRecommendation === "Package 355 - Owner Manual Evidence Review" && packageBundle.includes("Package 355 - Owner Manual Evidence Review"));
  ...
  check("soft launch remains no", config.model.softLaunchStatus === "NO");
  ...
  check("launch flags not approved", !/^\+.*(publicLaunchApproved:\s*true|ownerManualReviewRequired:\s*false|readyForProductionLaunch:\s*true|readyForLaunch:\s*true|ownerApprovalGranted:\s*true|screenshotsAccepted:\s*true|productionSafetyGreen:\s*true|softLaunchApproved:\s*true)/im.test(diffBundle));
