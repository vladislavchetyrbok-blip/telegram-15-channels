...
function buildConclusionLine(score, source) {
  ...
  if (source === "fast") return `${base}; быстрый режим показывает общий вектор пары.`;
  if (source === "personal") return `${base}; персональные даты делают расчёт чувствительнее к ритму каждого человека.`;
  return `${base}; точный режим остаётся rule-based MVP и помогает аккуратнее сравнить вводные.`;
}
