...
import {
  ...
  APHRODITE_BACKUP_FRESHNESS_RESTORE_REHEARSAL_PROTOCOL_TITLE,
  getAphroditeBackupFreshnessRestoreRehearsalProtocol,
} from "../lib/zodiac/aphrodite-backup-freshness-restore-rehearsal-protocol.ts";
...
console.log("Starting QA: Aphrodite Backup Freshness Restore Rehearsal Protocol...\n");

const modelPath = "../lib/zodiac/aphrodite-backup-freshness-restore-rehearsal-protocol.ts";
const dashboardPagePath = "../app/dashboard/networks/zodiac/backup-freshness-restore-rehearsal-protocol/page.tsx";
const docsPath = "../docs/aphrodite-backup-freshness-restore-rehearsal-protocol.md";
const reportPath = "../docs/aphrodite-package-reports/package-280.md";
...
check("title exported", model.title === APHRODITE_BACKUP_FRESHNESS_RESTORE_REHEARSAL_PROTOCOL_TITLE);
check("route exported", model.route === APHRODITE_BACKUP_FRESHNESS_RESTORE_REHEARSAL_PROTOCOL_ROUTE);
check("package number is 280", model.packageNumber === 280);
check("dashboard page uses readiness page", dashboardPageSource.includes("AphroditeReadinessPage"));
check("dashboard nav link exists", dashboardSource.includes(APHRODITE_BACKUP_FRESHNESS_RESTORE_REHEARSAL_PROTOCOL_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("backupFreshnessRestoreRehearsalProtocol"));
check("docs/report exist", docsSource.includes("Package 280") && reportSource.includes("Package 280"));
check("backup freshness remains blocked", model.backupFreshnessStatus === "BLOCKED_STALE_BACKUP");
...
console.log(`\nAphrodite Backup Freshness Restore Rehearsal Protocol QA complete: ${passed} passed, ${failed} failed.`);
