...
import {
  ...
  APHRODITE_PRODUCTION_ENV_BACKUP_READINESS_FIX_PLAN_TITLE,
  getAphroditeProductionEnvBackupReadinessFixPlan,
} from "../lib/zodiac/aphrodite-production-env-backup-readiness-fix-plan.ts";
...
console.log("Starting QA: Aphrodite Production Env Backup Readiness Fix Plan...\n");

const modelPath = "../lib/zodiac/aphrodite-production-env-backup-readiness-fix-plan.ts";
const dashboardPagePath = "../app/dashboard/networks/zodiac/production-env-backup-readiness-fix-plan/page.tsx";
const docsPath = "../docs/aphrodite-production-env-backup-readiness-fix-plan.md";
const reportPath = "../docs/aphrodite-package-reports/package-278.md";
...
check("title exported", model.title === APHRODITE_PRODUCTION_ENV_BACKUP_READINESS_FIX_PLAN_TITLE);
check("route exported", model.route === APHRODITE_PRODUCTION_ENV_BACKUP_READINESS_FIX_PLAN_ROUTE);
check("package number is 278", model.packageNumber === 278);
check("dashboard page uses readiness page", dashboardPageSource.includes("AphroditeReadinessPage"));
check("dashboard nav link exists", dashboardSource.includes(APHRODITE_PRODUCTION_ENV_BACKUP_READINESS_FIX_PLAN_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("productionEnvBackupReadinessFixPlan"));
check("docs/report exist", docsSource.includes("Package 278") && reportSource.includes("Package 278"));
check("productionReadinessStatus is blocked", model.productionReadinessStatus === "BLOCKED_MANUAL_SETUP_REQUIRED");
...
for (const blocker of ["DATABASE_URL missing", "TELEGRAM_BOT_TOKEN missing", "backup older than 24h"]) {
  check(`${blocker} blocker documented`, implementationBundle.includes(blocker) && model.remainingBlockers.includes(blocker));
}

for (const phrase of [
  "deployment provider / hosting env panel",
  "Local .env.local",
  ".env.local must never be committed",
  "Real secret values must never be pasted",
...
check("secret handling rules documented", model.secretHandlingRules.length >= 4 && implementationBundle.includes("secretHandlingRules"));
check("restore rehearsal checklist documented", model.restoreRehearsalChecklist.length >= 4 && implementationBundle.includes("restoreRehearsalChecklist"));
...
console.log(`\nAphrodite Production Env Backup Readiness Fix Plan QA complete: ${passed} passed, ${failed} failed.`);
