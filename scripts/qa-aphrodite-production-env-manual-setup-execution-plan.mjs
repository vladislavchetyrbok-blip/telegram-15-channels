...
} from "../lib/zodiac/aphrodite-production-env-manual-setup-execution-plan.ts";
...
console.log("Starting QA: Aphrodite Production Env Manual Setup Execution Plan...\n");

const modelPath = "../lib/zodiac/aphrodite-production-env-manual-setup-execution-plan.ts";
const pagePath = "../app/dashboard/networks/zodiac/production-env-manual-setup-execution-plan/page.tsx";
const docsPath = "../docs/aphrodite-production-env-manual-setup-execution-plan.md";
const reportPath = "../docs/aphrodite-package-reports/package-256.md";
...
check("title exported", model.title === "Production Env Manual Setup Execution Plan");
check("route exported", model.route === APHRODITE_PRODUCTION_ENV_MANUAL_SETUP_EXECUTION_PLAN_ROUTE);
check("package number is 256", model.packageNumber === 256);
check("dashboard page uses readiness page", pageSource.includes("AphroditeReadinessPage"));
check("navigation link exists in dashboard overview", dashboardSource.includes(APHRODITE_PRODUCTION_ENV_MANUAL_SETUP_EXECUTION_PLAN_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("productionEnvManualSetupExecutionPlan"));
check("docs/report exist", docsSource.includes("Package 256") && reportSource.includes("Package 256"));
...
check("next package recommendation documented", model.nextPackageRecommendation === "Package 257 - Backup Freshness Restore Rehearsal Execution Plan");
...
for (const required of [
  ...
  "Dry-run/live safety flags",
  "Launch approval flags",
  "Supabase envs if used",
  "Analytics envs if used",
  "ZODIAC_DASHBOARD_SESSION_SECRET legacy name",
]) {
  check(`env item documented: ${required}`, implementationBundle.includes(required));
}

check("DATABASE_URL documented as manual blocker", model.requiredProductionSecrets.some((item) => item.name === "DATABASE_URL" && item.currentStatus === "BLOCKED" && item.neverCommitValue === "Yes"));
check("TELEGRAM_BOT_TOKEN documented as manual blocker", model.requiredProductionSecrets.some((item) => item.name === "TELEGRAM_BOT_TOKEN" && item.currentStatus === "BLOCKED" && item.neverCommitValue === "Yes"));
check("APHRODITE_SESSION_SECRET documented", model.requiredProductionSecrets.some((item) => item.name === "APHRODITE_SESSION_SECRET" && item.currentStatus === "MANUAL REQUIRED"));
check("all env items use placeholders", [...model.requiredProductionSecrets, ...model.optionalEnvGroups].every((item) => item.safePlaceholderExample.includes("<") && item.safePlaceholderExample.includes(">")));
check("secret hygiene rules documented", hasAll(implementationBundle, [
...
check("leak response protocol documented", hasAll(implementationBundle, [
  "Stop work",
  "Rotate leaked token/secret",
  "Remove leaked value where possible",
  "Run secret scan/check",
]));
check("post-setup verification documented", hasAll(implementationBundle, [
  "git status -sb",
  "npm run typecheck",
  "npm run lint",
  "npm run build",
  ...
  "Verify Telegram bot token manually without sending messages",
  "Verify dashboard auth",
  "Verify Mini App public URL",
  "Verify no production launch happened",
]));
...
check("env secrets not configured flag", model.safetyFlags.envSecretsConfigured === false);
...
check("no secrets printed flag", model.safetyFlags.secretsPrinted === false);
...
const allowedChangedFiles = new Set([
  ...
  "app/dashboard/networks/zodiac/production-env-manual-setup-execution-plan/page.tsx",
...
check("only Package 256-scoped files changed", changedFiles.every((file) => allowedChangedFiles.has(file)));
