...
} from "../lib/zodiac/aphrodite-owner-manual-real-device-review-execution.ts";
...
console.log("Starting QA: Aphrodite Owner Manual Real-Device Review Execution...\n");

const modelPath = "../lib/zodiac/aphrodite-owner-manual-real-device-review-execution.ts";
const pagePath = "../app/dashboard/networks/zodiac/owner-manual-real-device-review-execution/page.tsx";
const docsPath = "../docs/aphrodite-owner-manual-real-device-review-execution.md";
const reportPath = "../docs/aphrodite-package-reports/package-253.md";
...
check("title exported", model.title === "Owner Manual Real-Device Review Execution");
check("route exported", model.route === APHRODITE_OWNER_MANUAL_REAL_DEVICE_REVIEW_EXECUTION_ROUTE);
check("package number is 253", model.packageNumber === 253);
check("dashboard page uses readiness page", pageSource.includes("AphroditeReadinessPage"));
check("dashboard route linked from overview", dashboardSource.includes(APHRODITE_OWNER_MANUAL_REAL_DEVICE_REVIEW_EXECUTION_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("ownerManualRealDeviceReviewExecution"));
check("docs/report exist", docsSource.includes("Package 253") && reportSource.includes("Package 253"));
check("owner review status required", model.ownerReviewStatus === "OWNER REVIEW REQUIRED" && implementationBundle.includes("OWNER REVIEW REQUIRED"));
...
check("next package recommendation documented", model.nextPackageRecommendation === "Package 254 — Telegram WebView Startapp Owner Review Execution" && implementationBundle.includes("Package 254 — Telegram WebView Startapp Owner Review Execution"));
...
check("checked viewports documented", hasAll(implementationBundle, [
  "360px",
  "390px",
  "430px",
]));

check("browser simulation status documented", implementationBundle.includes("PASS") && implementationBundle.includes("Browser Simulation"));
check("real-device manual requirements documented", implementationBundle.includes("MANUAL REQUIRED") && implementationBundle.includes("Real iPhone Safari") && implementationBundle.includes("Real Android Chrome"));
check("telegram webview manual requirements documented", implementationBundle.includes("Telegram iOS WebView") && implementationBundle.includes("Telegram Android WebView"));
...
const allowedChangedFiles = new Set([
  "lib/zodiac/aphrodite-owner-manual-real-device-review-execution.ts",
  "app/dashboard/networks/zodiac/owner-manual-real-device-review-execution/page.tsx",
  "scripts/qa-aphrodite-owner-manual-real-device-review-execution.mjs",
  "docs/aphrodite-owner-manual-real-device-review-execution.md",
  "docs/aphrodite-package-reports/package-253.md",
  "app/dashboard/networks/zodiac/page.tsx",
  "scripts/qa-zodiac-dashboard.mjs",
]);

check("git scope helper returned real change data", !changedFiles.includes("__git_diff_failed__"));
check("only Package 253-scoped files changed", changedFiles.every((file) => allowedChangedFiles.has(file)));
