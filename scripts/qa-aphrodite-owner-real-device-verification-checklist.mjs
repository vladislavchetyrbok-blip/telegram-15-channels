...
import {
  ...
  APHRODITE_OWNER_REAL_DEVICE_VERIFICATION_CHECKLIST_TITLE,
  getAphroditeOwnerRealDeviceVerificationChecklist,
} from "../lib/zodiac/aphrodite-owner-real-device-verification-checklist.ts";
...
console.log("Starting QA: Aphrodite Owner Real Device Verification Checklist...\n");

const modelPath = "../lib/zodiac/aphrodite-owner-real-device-verification-checklist.ts";
const dashboardPagePath = "../app/dashboard/networks/zodiac/owner-real-device-verification-checklist/page.tsx";
const docsPath = "../docs/aphrodite-owner-real-device-verification-checklist.md";
const reportPath = "../docs/aphrodite-package-reports/package-282.md";
...
check("title exported", model.title === APHRODITE_OWNER_REAL_DEVICE_VERIFICATION_CHECKLIST_TITLE);
check("route exported", model.route === APHRODITE_OWNER_REAL_DEVICE_VERIFICATION_CHECKLIST_ROUTE);
check("package number is 282", model.packageNumber === 282);
check("dashboard page uses readiness page", dashboardPageSource.includes("AphroditeReadinessPage"));
check("dashboard nav link exists", dashboardSource.includes(APHRODITE_OWNER_REAL_DEVICE_VERIFICATION_CHECKLIST_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("ownerRealDeviceVerificationChecklist"));
check("docs/report exist", docsSource.includes("Package 282") && reportSource.includes("Package 282"));
check("owner real device approval false", model.ownerRealDeviceApproval === false && implementationBundle.includes("ownerRealDeviceApproval = false"));
...
console.log(`\nAphrodite Owner Real Device Verification Checklist QA complete: ${passed} passed, ${failed} failed.`);
