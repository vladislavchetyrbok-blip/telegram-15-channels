...
function printPlan({ plan, renderer, mode, result = null }) {
  console.log("=== Social Phase 1 Auto MP4 Renderer ===");
  ...
  console.log("Technology         : sharp SVG/PNG frames + ffmpeg");
...
async function main() {
  ...
  const plan = createMp4RenderPlan(options);
  ...
  const result = await writeSocialMp4Bundle(options);
