...
check("shared helper range logic exists", typeof isBirthDateInAllowedRange === "function" && MIN_BIRTH_DATE_ISO === "1900-01-01" && typeof getTodayIsoDate() === "string" && typeof getCurrentYear() === "number");
...
const auditedBirthInputs = [
  { file: "../app/birth-matrix/BirthMatrixClient.tsx", kind: "native date picker (BirthMatrix / Matrix of Destiny page)" },
...
for (const { file, kind } of auditedBirthInputs) {
  ...
  const hasRange =
    ...
    /isBirthDateInAllowedRange/.test(src);
