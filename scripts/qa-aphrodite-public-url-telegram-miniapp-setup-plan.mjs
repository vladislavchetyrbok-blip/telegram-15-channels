...
import {
  ...
  APHRODITE_PUBLIC_URL_TELEGRAM_MINIAPP_SETUP_PLAN_TITLE,
  getAphroditePublicUrlTelegramMiniappSetupPlan,
} from "../lib/zodiac/aphrodite-public-url-telegram-miniapp-setup-plan.ts";
...
console.log("Starting QA: Aphrodite Public URL Telegram Mini App Setup Plan...\n");

const modelPath = "../lib/zodiac/aphrodite-public-url-telegram-miniapp-setup-plan.ts";
const dashboardPagePath = "../app/dashboard/networks/zodiac/public-url-telegram-miniapp-setup-plan/page.tsx";
const docsPath = "../docs/aphrodite-public-url-telegram-miniapp-setup-plan.md";
const reportPath = "../docs/aphrodite-package-reports/package-281.md";
...
check("title exported", model.title === APHRODITE_PUBLIC_URL_TELEGRAM_MINIAPP_SETUP_PLAN_TITLE);
check("route exported", model.route === APHRODITE_PUBLIC_URL_TELEGRAM_MINIAPP_SETUP_PLAN_ROUTE);
check("package number is 281", model.packageNumber === 281);
check("dashboard page uses readiness page", dashboardPageSource.includes("AphroditeReadinessPage"));
check("dashboard nav link exists", dashboardSource.includes(APHRODITE_PUBLIC_URL_TELEGRAM_MINIAPP_SETUP_PLAN_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("publicUrlTelegramMiniappSetupPlan"));
check("docs/report exist", docsSource.includes("Package 281") && reportSource.includes("Package 281"));
check("public URL setup is manual", model.publicUrlSetupStatus === "MANUAL REQUIRED");
check("BotFather setup not done", model.botFatherSetupStatus === "NOT_DONE");
check("route isolation PASS", model.routeIsolationStatus === "PASS");
...
for (const route of ["/miniapp", "/compatibility", "/birth-matrix", "/vip-preview", "/vip-compatibility-report"]) {
  check(`required public test route exists: ${route}`, implementationBundle.includes(route));
}
...
console.log(`\nAphrodite Public URL Telegram Mini App Setup Plan QA complete: ${passed} passed, ${failed} failed.`);
