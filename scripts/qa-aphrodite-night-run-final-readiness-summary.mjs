...
import {
  ...
  APHRODITE_NIGHT_RUN_FINAL_READINESS_SUMMARY_TITLE,
  getAphroditeNightRunFinalReadinessSummary,
} from "../lib/zodiac/aphrodite-night-run-final-readiness-summary.ts";
...
console.log("Starting QA: Aphrodite Night Run Final Readiness Summary...\n");

const modelPath = "../lib/zodiac/aphrodite-night-run-final-readiness-summary.ts";
const dashboardPagePath = "../app/dashboard/networks/zodiac/night-run-final-readiness-summary/page.tsx";
const docsPath = "../docs/aphrodite-night-run-final-readiness-summary.md";
const reportPath = "../docs/aphrodite-package-reports/package-286.md";
...
check("title exported", model.title === APHRODITE_NIGHT_RUN_FINAL_READINESS_SUMMARY_TITLE);
check("route exported", model.route === APHRODITE_NIGHT_RUN_FINAL_READINESS_SUMMARY_ROUTE);
check("package number is 286", model.packageNumber === 286);
check("dashboard page uses readiness page", dashboardPageSource.includes("AphroditeReadinessPage"));
check("dashboard nav link exists", dashboardSource.includes(APHRODITE_NIGHT_RUN_FINAL_READINESS_SUMMARY_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("nightRunFinalReadinessSummary"));
check("docs/report exist", docsSource.includes("Package 286") && reportSource.includes("Package 286"));
check("current branch recorded", model.currentBranch === "codex/night-run-packages-278-286-production-readiness");
check("current HEAD reporting rule recorded", model.currentHead === "resolved-by-final-git-report");
check("softLaunchStatus=NO", model.softLaunchStatus === "NO" && implementationBundle.includes("softLaunchStatus=NO"));
...
for (const field of [
  ...
  check(`model field exists: ${field}`, Array.isArray(value) && value.length > 0 && implementationBundle.includes(field));
}
...
for (const pkg of ["Package 278", "Package 279", "Package 280", "Package 281", "Package 282", "Package 283", "Package 284", "Package 285", "Package 286"]) {
  check(`completed package documented: ${pkg}`, implementationBundle.includes(pkg));
}
...
check("soft launch not approved flag", model.safetyFlags.softLaunchApproved === false);
...
console.log(`\nAphrodite Night Run Final Readiness Summary QA complete: ${passed} passed, ${failed} failed.`);
