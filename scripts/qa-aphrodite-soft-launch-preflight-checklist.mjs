...
import {
  ...
  APHRODITE_SOFT_LAUNCH_PREFLIGHT_CHECKLIST_TITLE,
  getAphroditeSoftLaunchPreflightChecklist,
} from "../lib/zodiac/aphrodite-soft-launch-preflight-checklist.ts";
...
console.log("Starting QA: Aphrodite Soft Launch Preflight Checklist...\n");

const modelPath = "../lib/zodiac/aphrodite-soft-launch-preflight-checklist.ts";
const pagePath = "../app/dashboard/networks/zodiac/soft-launch-preflight-checklist/page.tsx";
const docsPath = "../docs/aphrodite-soft-launch-preflight-checklist.md";
const reportPath = "../docs/aphrodite-package-reports/package-249.md";
...
check("title exported", model.title === APHRODITE_SOFT_LAUNCH_PREFLIGHT_CHECKLIST_TITLE);
check("route exported", model.route === APHRODITE_SOFT_LAUNCH_PREFLIGHT_CHECKLIST_ROUTE);
check("package number is 249", model.packageNumber === 249);
check("dashboard page uses readiness page", pageSource.includes("AphroditeReadinessPage"));
check("dashboard route linked from overview", dashboardSource.includes(APHRODITE_SOFT_LAUNCH_PREFLIGHT_CHECKLIST_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("softLaunchPreflightChecklist"));
check("docs/report exist", docsSource.includes("Package 249") && reportSource.includes("Package 249"));
...
check("next package recommendation documented", model.nextPackageRecommendation === "Package 250 - Owner Manual Review Pack" && implementationBundle.includes("Package 250 - Owner Manual Review Pack"));
...
for (const category of [
  ...
  check(`checklist category present: ${category}`, implementationBundle.includes(category));
}

check("code checks include required commands", hasAll(implementationBundle, [
  "typecheck",
  "lint",
  "build",
...
check("production env blockers present", hasAll(implementationBundle, [
...
check("backup/restore blockers present", hasAll(implementationBundle, [
  "backup <24h",
  "restore rehearsal",
  "rollback point",
...
check("real-device QA checks present", hasAll(implementationBundle, [
  "iPhone Safari",
  "Android Chrome",
...
check("Telegram WebView/startapp checks present", hasAll(implementationBundle, [
  "startapp present",
...
check("content/CTA review checks present", hasAll(implementationBundle, [
  "home CTA",
  "compatibility CTA",
  "Birth Matrix CTA",
  "Mystic Cards CTA",
...
check("safety checks present", hasAll(implementationBundle, [
...
for (const stopCondition of [
  ...
  check(`stop condition present: ${stopCondition}`, model.stopConditions.some((item) => item.area === stopCondition) && implementationBundle.includes(stopCondition));
}

check("relevant QA scripts listed", hasAll(implementationBundle, [
  "qa-aphrodite-soft-launch-preflight-checklist.mjs",
  "qa-aphrodite-soft-launch-scope-selector.mjs",
  "qa-aphrodite-soft-launch-owner-go-no-go-gate.mjs",
...
const allowedChangedFiles = new Set([
  "lib/zodiac/aphrodite-soft-launch-preflight-checklist.ts",
  "app/dashboard/networks/zodiac/soft-launch-preflight-checklist/page.tsx",
  "scripts/qa-aphrodite-soft-launch-preflight-checklist.mjs",
  "docs/aphrodite-soft-launch-preflight-checklist.md",
  "docs/aphrodite-package-reports/package-249.md",
  "app/dashboard/networks/zodiac/page.tsx",
  "scripts/qa-zodiac-dashboard.mjs",
]);

check("git scope helper returned real change data", !changedFiles.includes("__git_diff_failed__"));
check("only Package 249-scoped files changed", changedFiles.every((file) => allowedChangedFiles.has(file)));
