...
import {
  ...
  APHRODITE_SOFT_LAUNCH_DRY_RUN_ROLLBACK_PLAN_TITLE,
  getAphroditeSoftLaunchDryRunRollbackPlan,
} from "../lib/zodiac/aphrodite-soft-launch-dry-run-rollback-plan.ts";
...
console.log("Starting QA: Aphrodite Soft Launch Dry Run and Rollback Plan...\n");

const modelPath = "../lib/zodiac/aphrodite-soft-launch-dry-run-rollback-plan.ts";
const dashboardPagePath = "../app/dashboard/networks/zodiac/soft-launch-dry-run-rollback-plan/page.tsx";
const docsPath = "../docs/aphrodite-soft-launch-dry-run-rollback-plan.md";
const reportPath = "../docs/aphrodite-package-reports/package-283.md";
...
check("title exported", model.title === APHRODITE_SOFT_LAUNCH_DRY_RUN_ROLLBACK_PLAN_TITLE);
check("route exported", model.route === APHRODITE_SOFT_LAUNCH_DRY_RUN_ROLLBACK_PLAN_ROUTE);
check("package number is 283", model.packageNumber === 283);
check("dashboard page uses readiness page", dashboardPageSource.includes("AphroditeReadinessPage"));
check("dashboard nav link exists", dashboardSource.includes(APHRODITE_SOFT_LAUNCH_DRY_RUN_ROLLBACK_PLAN_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("softLaunchDryRunRollbackPlan"));
check("docs/report exist", docsSource.includes("Package 283") && reportSource.includes("Package 283"));
check("soft launch not approved", model.softLaunchStatus === "NOT_APPROVED");
...
check("no env changes flag", model.safetyFlags.envChangesAdded === false);
...
console.log(`\nAphrodite Soft Launch Dry Run and Rollback Plan QA complete: ${passed} passed, ${failed} failed.`);
