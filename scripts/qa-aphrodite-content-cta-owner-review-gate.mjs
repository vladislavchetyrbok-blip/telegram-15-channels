...
import {
  ...
  APHRODITE_CONTENT_CTA_OWNER_REVIEW_GATE_TITLE,
  getAphroditeContentCtaOwnerReviewGate,
} from "../lib/zodiac/aphrodite-content-cta-owner-review-gate.ts";
...
console.log("Starting QA: Content CTA Owner Review Gate...\n");

const modelPath = "../lib/zodiac/aphrodite-content-cta-owner-review-gate.ts";
const pagePath = "../app/dashboard/networks/zodiac/content-cta-owner-review-gate/page.tsx";
const docsPath = "../docs/aphrodite-content-cta-owner-review-gate.md";
const reportPath = "../docs/aphrodite-package-reports/package-233.md";
...
check("title exported", model.title === APHRODITE_CONTENT_CTA_OWNER_REVIEW_GATE_TITLE);
check("route exported", model.route === APHRODITE_CONTENT_CTA_OWNER_REVIEW_GATE_ROUTE);
check("package number is 233", model.packageNumber === 233);
check("dashboard route exists", pageSource.includes("getAphroditeContentCtaOwnerReviewGate") && pageSource.includes("AphroditeReadinessPage"));
check("dashboard route linked from overview", dashboardSource.includes(APHRODITE_CONTENT_CTA_OWNER_REVIEW_GATE_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("contentCtaOwnerReviewGate"));
check("docs/report exists", docsSource.includes("Package 233") && reportSource.includes("Package 233"));
...
for (const area of [
  ...
  check(`CTA area exists: ${area}`, model.items.some((item) => item.area === area));
  check(`CTA area rendered/documented: ${area}`, implementationBundle.includes(area));
}

check("active CTA logic not changed wording", /Active CTA logic was not changed/i.test(implementationBundle));
check("no publish scripts changed wording", /No publish scripts changed/i.test(implementationBundle));
check("no Telegram messages sent wording", /No Telegram messages were sent/i.test(implementationBundle));
check("owner review required wording", /Owner review required/i.test(implementationBundle));
check("public launch not approved wording", /Public launch not approved/i.test(implementationBundle));
check("every CTA item says active logic changed No", model.items.every((item) => item.activeLogicChanged === "No"));
