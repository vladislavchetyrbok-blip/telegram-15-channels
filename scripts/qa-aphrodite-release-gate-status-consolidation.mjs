...
import {
  ...
  APHRODITE_RELEASE_GATE_STATUS_CONSOLIDATION_TITLE,
  getAphroditeReleaseGateStatusConsolidation,
} from "../lib/zodiac/aphrodite-release-gate-status-consolidation.ts";
...
console.log("Starting QA: Aphrodite Release Gate Status Consolidation...\n");

const modelPath = "../lib/zodiac/aphrodite-release-gate-status-consolidation.ts";
const dashboardPagePath = "../app/dashboard/networks/zodiac/release-gate-status-consolidation/page.tsx";
const docsPath = "../docs/aphrodite-release-gate-status-consolidation.md";
const reportPath = "../docs/aphrodite-package-reports/package-284.md";
...
check("title exported", model.title === APHRODITE_RELEASE_GATE_STATUS_CONSOLIDATION_TITLE);
check("route exported", model.route === APHRODITE_RELEASE_GATE_STATUS_CONSOLIDATION_ROUTE);
check("package number is 284", model.packageNumber === 284);
check("dashboard page uses readiness page", dashboardPageSource.includes("AphroditeReadinessPage"));
check("dashboard nav link exists", dashboardSource.includes(APHRODITE_RELEASE_GATE_STATUS_CONSOLIDATION_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("releaseGateStatusConsolidation"));
check("docs/report exist", docsSource.includes("Package 284") && reportSource.includes("Package 284"));
check("release gate remains blocked", model.releaseGateStatus === "BLOCKED");
...
for (const phrase of [
  ...
  check(`required gate exists: ${phrase}`, implementationBundle.includes(phrase));
}
...
console.log(`\nAphrodite Release Gate Status Consolidation QA complete: ${passed} passed, ${failed} failed.`);
