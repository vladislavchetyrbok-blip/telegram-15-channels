...
import {
  ...
  APHRODITE_PRODUCTION_ENV_SETUP_PROTOCOL_TITLE,
  getAphroditeProductionEnvSetupProtocol,
} from "../lib/zodiac/aphrodite-production-env-setup-protocol.ts";
...
console.log("Starting QA: Production Env Setup Protocol...\n");

const modelPath = "../lib/zodiac/aphrodite-production-env-setup-protocol.ts";
const pagePath = "../app/dashboard/networks/zodiac/production-env-setup-protocol/page.tsx";
const docsPath = "../docs/aphrodite-production-env-setup-protocol.md";
const reportPath = "../docs/aphrodite-package-reports/package-229.md";
...
const envExampleSource = exists(envExamplePath) ? read(envExamplePath) : "";
const model = getAphroditeProductionEnvSetupProtocol();
const implementationBundle = [modelSource, pageSource, docsSource, reportSource, dashboardSource, dashboardQaSource, envExampleSource].join("\n");
const safetyBundle = [modelSource, pageSource, docsSource, reportSource].join("\n");

check("title exported", model.title === APHRODITE_PRODUCTION_ENV_SETUP_PROTOCOL_TITLE);
check("route exported", model.route === APHRODITE_PRODUCTION_ENV_SETUP_PROTOCOL_ROUTE);
check("package number is 229", model.packageNumber === 229);
check("dashboard route exists", pageSource.includes("getAphroditeProductionEnvSetupProtocol") && pageSource.includes("AphroditeReadinessPage"));
check("dashboard route linked from overview", dashboardSource.includes(APHRODITE_PRODUCTION_ENV_SETUP_PROTOCOL_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("productionEnvSetupProtocol"));
check("docs/report exists", docsSource.includes("Package 229") && reportSource.includes("Package 229"));
...
for (const required of [
  ...
  check(`protocol covers ${required}`, implementationBundle.includes(required));
}

check("DATABASE_URL remains manual blocker", /DATABASE_URL.*manual setup|DATABASE_URL.*manual configuration/i.test(implementationBundle));
check("TELEGRAM_BOT_TOKEN remains manual blocker", /TELEGRAM_BOT_TOKEN.*manual setup|TELEGRAM_BOT_TOKEN.*manual configuration/i.test(implementationBundle));
check("backup freshness manual blocker", /backup freshness.*manual launch blocker|backup freshness.*<24h/i.test(implementationBundle));
check("owner approval required", /owner explicit approval|owner manual review/i.test(implementationBundle));
check("manual setup wording exists", /configured manually|manual setup|ownerAction/i.test(implementationBundle));
check("placeholders only in env example", /<set-in-secret-store-only>|<long-random-dashboard-session-secret>|<public-app-url>/.test(envExampleSource));

for (const status of ["MANUAL REQUIRED", "BLOCKED", "DOCUMENTED"]) {
