...
import {
  ...
  APHRODITE_LAUNCH_SIMULATION_STATUS_REPORT_TITLE,
  getAphroditeLaunchSimulationStatusReport,
} from "../lib/zodiac/aphrodite-launch-simulation-status-report.ts";
...
console.log("Starting QA: Launch Simulation Status Report...\n");

const modelPath = "../lib/zodiac/aphrodite-launch-simulation-status-report.ts";
const pagePath = "../app/dashboard/networks/zodiac/launch-simulation-status-report/page.tsx";
const docsPath = "../docs/aphrodite-launch-simulation-status-report.md";
const reportPath = "../docs/aphrodite-package-reports/package-234.md";
...
check("title exported", model.title === APHRODITE_LAUNCH_SIMULATION_STATUS_REPORT_TITLE);
check("route exported", model.route === APHRODITE_LAUNCH_SIMULATION_STATUS_REPORT_ROUTE);
check("package number is 234", model.packageNumber === 234);
check("dashboard route exists", pageSource.includes("getAphroditeLaunchSimulationStatusReport") && pageSource.includes("AphroditeReadinessPage"));
check("dashboard route linked from overview", dashboardSource.includes(APHRODITE_LAUNCH_SIMULATION_STATUS_REPORT_ROUTE));
check("dashboard QA route exists", dashboardQaSource.includes("launchSimulationStatusReport"));
check("docs/report exists", docsSource.includes("Package 234") && reportSource.includes("Package 234"));
...
for (const section of [
  ...
  check(`readiness section exists: ${section}`, model.sections.some((item) => item.area === section));
  check(`readiness section rendered/documented: ${section}`, implementationBundle.includes(section));
}

for (const status of ["PASS", "BLOCKED", "MANUAL REQUIRED", "NOT CHECKED", "OWNER REVIEW REQUIRED"]) {
  check(`status category exists: ${status}`, model.statusCategories.includes(status) && implementationBundle.includes(status));
}

check("launch not approved wording", /Launch not approved|launch not approved/i.test(implementationBundle));
check("no production launch wording", /No production launch/i.test(implementationBundle));
check("no Telegram API wording", /No Telegram API call was made|Telegram API used: No/i.test(implementationBundle));
check("no DB write wording", /No DB write was added|DB write added: No/i.test(implementationBundle));
check("no payments/VIP wording", /No payment or VIP unlock was added|Payment added: No/i.test(implementationBundle));
check("no cron/workflow/publish wording", /No cron, workflow or publish script was changed|Cron\/workflows\/publish scripts changed: No/i.test(implementationBundle));
