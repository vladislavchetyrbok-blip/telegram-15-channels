...
### Stage 1: Visual Asset Production & Import
1. Generate the visual assets (Avatars, Covers, Daily Placeholders) using your preferred AI tool. 
   *(Use the prompts generated by `npm run zodiac:export-image-prompts` or check `docs/ZODIAC_IMAGE_PROMPTS_COPYPASTE.md`).*
2. Save the files to the correct locations in `public/assets/zodiac/`.
3. Read `docs/ZODIAC_VISUAL_ASSET_IMPORT_CHECKLIST.md` for format and sizing rules.
4. Validate that all assets are correctly named and present by running:
   ```bash
   npm run zodiac:validate-assets
   ```

### Stage 2: Create 13 Telegram Channels Manually
