...
import {
  APHRODITE_FUTURE_TIMELINE_PROMISE,
  APHRODITE_FUTURE_TIMELINE_SAFETY_BOUNDARIES,
...
} from "@/lib/zodiac/aphrodite-ai-future-timeline-foundation";
...
const freePreview = [
  `Main theme: ${preview.mainTheme}`,
  `One love signal: ${preview.loveSignal}`,
  `One opportunity signal: ${preview.opportunitySignal}`,
  `One warning period: ${preview.warningPeriod}`,
  `One best action window: ${preview.bestActionWindow}`,
  `One next step: ${preview.nextStep}`,
];

export default function AiFutureTimelineFoundationPage() {
  return (
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        <header className="space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-xs text-rose-300">
            ...
            <span>Aphrodite / AI Future Timeline</span>
          </div>
          <h1 className="text-3xl font-light text-white tracking-tight">AI Future Timeline Foundation</h1>
          <p className="text-rose-300/90 text-sm font-medium">
            Local foundation only / No AI API / No payment / No real VIP unlock
          </p>
          <p className="text-slate-400 max-w-3xl text-lg">
            The local, deterministic foundation for the AI Future Timeline. It gently surfaces possible emotional
            windows ahead. There is no real AI here — the word describes the product, not the implementation. It
            never predicts exact dates or guaranteed events, and never gives financial, medical, legal, or
            emergency advice.
          </p>
          <div className="bg-rose-950/20 border border-rose-900/40 rounded-lg p-4">
            ...
            <p className="text-rose-100 text-base">{APHRODITE_FUTURE_TIMELINE_PROMISE}</p>
          </div>
          <div className="flex flex-wrap gap-2 text-xs">
            {APHRODITE_FUTURE_TIMELINE_SAFETY_BOUNDARIES.map((b) => (
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
          ...
          <p className="text-sm text-slate-500">Deterministic sample (Leo, horizon: {preview.visibleHorizon}). No data is stored.</p>
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
          <h2 className="text-xl font-medium text-white">Timeline periods</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {periods.map((p) => (
              <div key={p.id} className="border border-slate-800 rounded-lg p-4 space-y-2">
                <div className="text-white text-sm font-medium">{p.label}</div>
                <p className="text-sm text-slate-300">{p.theme}</p>
                <p className="text-xs text-slate-400">{p.loveSignal}</p>
                <p className="text-xs text-slate-400">{p.bestActionWindow}</p>
                ...
                <ul className="list-disc pl-5 text-xs text-slate-400 space-y-0.5">
                  {p.futureVipDepth.map((v) => (<li key={v}>{v}</li>))}
                </ul>
                {p.safetyNote && <p className="text-[11px] text-amber-400/80">{p.safetyNote}</p>}
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-3">
          ...
          <p className="text-sm text-slate-300">Package 140 — Social Traffic Layer Architecture.</p>
        </section>
