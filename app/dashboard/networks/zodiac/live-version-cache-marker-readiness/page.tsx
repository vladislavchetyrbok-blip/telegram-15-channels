...
export default function AphroditeLiveVersionCacheMarkerReadinessPage() {
  return (
    <div
      className="min-h-screen overflow-x-hidden bg-black px-4 py-6 text-slate-200 sm:px-6 lg:px-8"
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {model.markerChecks.map((check) => (
              <article key={check.id} className="min-w-0 rounded-lg border border-slate-800 bg-black/30 p-4">
                ...
                <p className="mt-1 break-words font-mono text-xs text-slate-500">{check.target}</p>
                <p className="mt-3 break-all text-xs text-cyan-200">{check.markerOrSignal}</p>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {model.routeMarkers.map((route) => (
              <article key={route.id} className="min-w-0 rounded-lg border border-slate-800 bg-black/30 p-4">
                <code className="max-w-full break-all rounded border border-slate-800 bg-slate-950 px-2 py-1 text-xs text-cyan-200">{route.route}</code>
