...
import { ClipboardCheck, RotateCcw, Rocket, ShieldAlert, ShieldCheck } from "lucide-react";
import type { ReactNode } from "react";

import {
  APHRODITE_MANUAL_LAUNCH_RUNBOOK_ROLLBACK_PACK_ROUTE,
  getAphroditeManualLaunchRunbookRollbackPack,
  type AphroditeManualLaunchRunbookStatus,
} from "@/lib/zodiac/aphrodite-manual-launch-runbook-rollback-pack";
...
export default function AphroditeManualLaunchRunbookRollbackPackPage() {
  return (
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="space-y-4">
          <div className="inline-flex max-w-full flex-wrap items-center gap-2 rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-xs leading-5 text-emerald-300">
            ...
            <span>Aphrodite / Manual Launch Runbook & Rollback / Package {model.packageNumber}</span>
          </div>
          ...
          <p className="max-w-5xl text-base leading-7 text-slate-400 sm:text-lg sm:leading-8">
            Final static runbook before owner go/no-go. It explains required checks, manual approval, launch sequencing, rollback, monitoring, incident response, and current blockers without performing launch.
          </p>
          <div className="rounded-lg border border-rose-900/50 bg-rose-950/20 px-4 py-3 text-sm leading-6 text-rose-100">
            {model.launchNotPerformedWording}
          </div>
        </header>
        ...
        <ReviewSection title="status legend" icon={<ShieldAlert className="h-5 w-5 text-amber-200" />}>
          <div className="flex flex-wrap gap-2">
            {model.statusLegend.map((status) => (
        ...
        <ReviewSection title="manual launch runbook sections" icon={<ClipboardCheck className="h-5 w-5 text-cyan-400" />}>
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
          <ReviewSection title="required pre-launch checks" icon={<ClipboardCheck className="h-5 w-5 text-cyan-400" />}>
            <ul className="grid gap-2 text-sm leading-6 text-slate-300">
              {model.requiredPreLaunchChecks.map((check) => (
                <li key={check} className="rounded-md border border-amber-900/40 bg-amber-950/20 px-3 py-2 text-amber-100">
                  {check}
                </li>
              ))}
            </ul>
          ...
          <ReviewSection title="rollback plan" icon={<RotateCcw className="h-5 w-5 text-cyan-400" />}>
            <ul className="grid gap-2 text-sm leading-6 text-slate-300">
              {model.rollbackPlan.map((step) => (
                <li key={step} className="rounded-md border border-cyan-900/40 bg-cyan-950/20 px-3 py-2 text-cyan-100">
                  {step}
                </li>
              ))}
            </ul>
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
          <ReviewSection title="current blockers" icon={<ShieldAlert className="h-5 w-5 text-amber-200" />}>
            <ul className="grid gap-2 text-sm leading-6 text-slate-300">
        ...
        <div className="border-t border-slate-800/50 pt-4">
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            ...
            <Link href="/dashboard/networks/zodiac/production-env-handoff-checklist" className="inline-flex rounded-md border border-indigo-900/40 bg-indigo-950/20 px-3 py-2 text-indigo-300 underline underline-offset-4 hover:border-indigo-700 hover:text-indigo-200">Production Env Handoff</Link>
          ...
          <p className="mt-3 break-all font-mono text-[11px] leading-5 text-slate-500">{APHRODITE_MANUAL_LAUNCH_RUNBOOK_ROLLBACK_PACK_ROUTE}</p>
        </div>
      </div>
    </div>
  );
...
function statusClassName(status: AphroditeManualLaunchRunbookStatus) {
  if (status === "LAUNCH NOT PERFORMED" || status === "NOT APPROVED") return "inline-flex max-w-full break-words rounded border border-rose-900/50 bg-rose-950/30 px-2 py-1 text-xs leading-5 text-rose-200";
  if (status === "OWNER REVIEW REQUIRED" || status === "READY FOR OWNER REVIEW") return "inline-flex max-w-full break-words rounded border border-violet-900/50 bg-violet-950/30 px-2 py-1 text-xs leading-5 text-violet-200";
  if (status === "BLOCKED BY BACKUP" || status === "BLOCKED BY ENV" || status === "BLOCKED BY MANUAL QA") return "inline-flex max-w-full break-words rounded border border-amber-900/50 bg-amber-950/30 px-2 py-1 text-xs leading-5 text-amber-200";
  return "inline-flex max-w-full break-words rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs leading-5 text-slate-300";
}
