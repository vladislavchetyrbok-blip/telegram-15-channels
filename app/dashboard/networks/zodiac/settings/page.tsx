...
import { Settings, Shield, Server, Box, Key, Zap, CheckCircle2, Lock, ListTodo, Layers, Link as LinkIcon, Database, Activity, ExternalLink, RefreshCw } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <main className="mx-auto mt-8 max-w-7xl space-y-10 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <section data-qa="settings-entry-points" className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4 font-mono text-xs">
              <div className="rounded border border-slate-100 bg-slate-50 p-3">
                <div className="mb-1 font-semibold text-slate-500 font-sans text-xs">Zodiac Mini:</div>
                <div className="break-all text-slate-800">https://t.me/zodiac_love_check_bot?startapp=compat</div>
              </div>
              
              <div className="rounded border border-slate-100 bg-slate-50 p-3">
                <div className="mb-1 font-semibold text-slate-500 font-sans text-xs">Zodiac Control (Overview):</div>
                <div className="break-all text-slate-800">https://telegram-15-channels.vercel.app/dashboard/networks/zodiac</div>
              </div>

              <div className="rounded border border-slate-100 bg-slate-50 p-3">
                <div className="mb-1 font-semibold text-slate-500 font-sans text-xs">Zodiac Pulse (Analytics):</div>
                <div className="break-all text-slate-800">https://telegram-15-channels.vercel.app/dashboard/networks/zodiac/analytics</div>
              </div>

              <div className="rounded border border-slate-100 bg-slate-50 p-3">
                <div className="mb-1 font-semibold text-slate-500 font-sans text-xs">Launch Control:</div>
                <div className="break-all text-slate-800">https://telegram-15-channels.vercel.app/dashboard/networks/zodiac/launch</div>
              </div>
              
              <div className="flex flex-wrap gap-2 pt-2">
                <Link href="/dashboard/login" className="inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-1 font-sans text-xs font-medium text-slate-600 hover:bg-slate-200"><ExternalLink className="h-3 w-3"/> /dashboard/login</Link>
                <Link href="/dashboard/networks/zodiac/feedback" className="inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-1 font-sans text-xs font-medium text-slate-600 hover:bg-slate-200"><ExternalLink className="h-3 w-3"/> feedback</Link>
                <Link href="/dashboard/networks/zodiac/publishing" className="inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-1 font-sans text-xs font-medium text-slate-600 hover:bg-slate-200"><ExternalLink className="h-3 w-3"/> publishing</Link>
                <Link href="/dashboard/networks/zodiac/settings" className="inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-1 font-sans text-xs font-medium text-slate-600 hover:bg-slate-200"><ExternalLink className="h-3 w-3"/> settings</Link>
              </div>
            </div>
          </section>

          {/* Vercel Env Checklist */}
          <section data-qa="settings-vercel-env" className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-3">
              <div className="flex flex-col gap-1 rounded border border-slate-100 bg-slate-50 p-3">
                ...
                <span className="text-xs text-slate-500">Включает gate на `/dashboard`. Required before 20.</span>
              </div>
              
              <div className="flex flex-col gap-1 rounded border border-slate-100 bg-slate-50 p-3">
                <code className="text-sm font-semibold text-slate-900">ZODIAC_DASHBOARD_ADMIN_PASSWORD_SHA256</code>
                <span className="text-xs text-slate-500">Хэш пароля. Required before 20.</span>
              </div>

              <div className="flex flex-col gap-1 rounded border border-slate-100 bg-slate-50 p-3">
                <code className="text-sm font-semibold text-slate-900">ZODIAC_DASHBOARD_SESSION_SECRET</code>
                <span className="text-xs text-slate-500">Секрет для кук. Required before 20.</span>
              </div>

              <div className="flex flex-col gap-1 rounded border border-slate-100 bg-slate-50 p-3">
                <code className="text-sm font-semibold text-slate-900">ZODIAC_ANALYTICS_REDIS_URL</code>
                <span className="text-xs text-slate-500">Upstash URL. Required before first 5.</span>
              </div>

              <div className="flex flex-col gap-1 rounded border border-slate-100 bg-slate-50 p-3">
                <code className="text-sm font-semibold text-slate-900">ZODIAC_ANALYTICS_REDIS_TOKEN</code>
                <span className="text-xs text-slate-500">Upstash Token. Required before first 5.</span>
              </div>
            </div>
          </section>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          {/* Mode Matrix */}
          <section data-qa="settings-mode-matrix" className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm overflow-x-auto">
          ...
          <section data-qa="settings-manual-actions" className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-2">
              {manualChecklist.map((action, i) => (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-100 transition-colors"
                >
                  <div className={`flex h-5 w-5 items-center justify-center rounded border ${manualActions[action.id] ? "border-emerald-500 bg-emerald-500 text-white" : "border-slate-300 bg-white"}`}>
